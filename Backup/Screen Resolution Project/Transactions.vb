...
Public Class Transactions
        ...
        Try
            ...
            If TXN_Code = "POSINV" Then
                ...
                Else
                    ...
                    stQueryI = stQueryI & "POS_INVOICE_ITEM_LOG.NEXTVAL" & "," & INVHSYSID.ToString & ",'" & itemDetails(1) & "','" & itemDetails(2).Replace("'", "''") & "','" & itemDetails(10) & "'," & Convert.ToInt32(itemDetails(3).ToString).ToString & "," & ((Convert.ToInt32(itemDetails(3).ToString) * Convert.ToDouble(itemDetails(4).ToString)) - Convert.ToDouble(itemDetails(6).ToString)).ToString & ",'" & itemDetails(17).ToString & "'," & Convert.ToDouble(itemDetails(4).ToString).ToString & ",0," & Convert.ToDouble(itemDetails(11).ToString).ToString & "," & Convert.ToDouble(itemDetails(6).ToString).ToString & ",'','" & itemDetails(12).ToString & "','" & itemDetails(13).ToString & "','" & itemDetails(9).ToString & "'," & Convert.ToDouble(itemDetails(4).ToString).ToString & ",'" & itemDetails(23).ToString & "','" & itemDetails(21).ToString & "','+','" & itemDetails(14).ToString & "','" & itemDetails(24).ToString & "',"
                    ...
                    If count > 0 Then
                        ...
                        For k = 1 To txtItemCode.Count
                            ...
                            If dsI.Tables("Table").Rows.Count > 0 Then
                                ...
                                stQuery = stQuery & maxItemSYSID & "," & maxSYS_ID & "," & rowI.Item(0).ToString & ",'" & rowI.Item(2).ToString & "','" & dsIN.Tables("Table").Rows.Item(0).Item(0).ToString() & "','" & rowI.Item(4).ToString.Replace("'", "''") & "','" & rowI.Item(5).ToString & "'," & ItmQtyFound(0).Text & ",0," & ItmQtyFound(0).Text & ",0,0," & rowI.Item(6).ToString & ",0,0," & Convert.ToDouble(ItmQtyFound(0).Text) * Convert.ToDouble(ItmPriceFound(0).Text) & "," & Convert.ToDouble(ItmQtyFound(0).Text) * Convert.ToDouble(ItmPriceFound(0).Text) & ",'Y','" & Location_Code & "','" & Location_Code & "','" & txtSalesmanCode.Text & "','" & rowI.Item(7).ToString & "','" & rowI.Item(8).ToString & "','" & rowI.Item(2).ToString & "','','','" & LogonUser & "',current_timestamp)"
    ...
    Public Sub PrintDINVReport()
        Try
            ...
            stQuery = stQuery + " nvl((select ITED_FC_AMT from OT_INVOICE_ITEM_TED where ITED_H_SYS_ID=INVH_SYS_ID and ITED_TED_HEAD_ITEM_NUM=1 and ITED_TED_TYPE_NUM=(select TED_TAX_DISC_EXP_NUM from OM_TED_TYPE where TED_TYPE_CODE='TEDDIS')),0) as totdisamt "
            stQuery = stQuery + " from "
    ...
    Private Shared Sub RenderLogo(ByVal bw As BinaryWriter)
        'Dim data = GetBitmapData("logo15.jpg")
        'Dim dots = data.Dots
        ...
        displayString = displayString + Chr(&HD) + ESC + Chr(&H45) + "1" + ESC + Chr(&H61) + "1" + "ALJABER GALLERY"
    ...
    Private Sub LoadHoldedTransactionsDetials(ByVal transno As String, ByVal transtype As String)
        Try
            ...
            stQuery = "select INVH_SYS_ID,INVH_NO,PRODCODE,PRODDESC,PRODQTY,PRODRATE,PRODDISCCODE,PRODDISCAMT,PRODEXPCODE,PRODEXPAMT,PRODPRICE,INVH_CUST_CODE || ' - ' || INVH_CUST_NAME,INVH_SM_CODE from OT_POS_INVOICE_ITEM_LOG,OT_POS_INVOICE_HEAD_LOG where PROD_INVI_INVH_SYS_ID=INVH_SYS_ID and INVH_NO=" & transno
