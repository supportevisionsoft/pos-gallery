...
Public Class LoginForm
    ...
    Private Sub LoginForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            '        If NowDate >= stDate And NowDate <= endDate Then
            '            val = True
            '        End If
            '        If val = True Then
            '            'loginfunc()
            ...
            'POSCounterName = "GATANAPC"

            'MsgBox("You are login as 099 location.. Testing on process")
            ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        ...
        Catch ex As Exception
            ...
            ElseIf ex.Message.GetHashCode = 1199841023 Then
                MsgBox("TNS no listner!", MsgBoxStyle.Critical, "Database Error")
                End
            End If
    ...
    Private Sub txtPassword_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtPassword.LostFocus
        Try
            ...
            If Not ds.Tables("Table").Rows.Count > 0 Then
                ...
                End If
            Else
                stQuery = "SELECT USER_PASSWD FROM MENU_USER WHERE USER_ID = '" & txtUserName.Text & "' AND USER_FIELD_05 ='" & txtPassword.Text & "'"
                ds = db.SelectFromTableODBC(stQuery)
                If Not ds.Tables("Table").Rows.Count > 0 Then
                    If Not txtPassword.Text = "" Then
                        txtPassword.Text = ""
                ...
                Else
                    ...
                    Else
                        ...
                        Else
                            ...
                            '''stQuery = "select POSCNT_NO,OM_POS_COUNTER.ROWID from om_pos_counter where poscnt_locn_code='" & Location_Code & "' and poscnt_frz_flag_num = 2  and (POSCNT_COMPUTER_NAME='IBNEGYPTPC' or POSCNT_IP_ADDRESS= '')"
                            ' BELOW LINE COMMENTED BY KASHIF
                            'stQuery = "select POSCNT_NO,OM_POS_COUNTER.ROWID from om_pos_counter where poscnt_locn_code='" & Location_Code & "' and poscnt_frz_flag_num = 2  and (POSCNT_COMPUTER_NAME='GMADINATPC' or POSCNT_IP_ADDRESS= '')"
                            ' BELOW LINE ADDED BY KASHIF
                            stQuery = "select POSCNT_NO,OM_POS_COUNTER.ROWID from om_pos_counter where poscnt_locn_code='" & Location_Code & "' and poscnt_frz_flag_num = 2  and (POSCNT_COMPUTER_NAME='" + POSCounterName + "' or POSCNT_IP_ADDRESS= '')"
            ...
            Else
                ...
                If posReportHeadTitles.Count > 0 Then
                    ...
                    If posReportHeadTitles.Contains("Transaction") = True Then
                        ...
                        stQuery = "Select distinct PSD_REPORT_TITLE from pos_report_detail where PSD_REPORT_ID in (select USMA_SUB_REPORT_ID from USER_SUBMENU_ACCESS where USMA_PRIV_YN='Y' AND USMA_UG_ID=(SELECT USER_GROUP_ID FROM MENU_USER WHERE USER_ID = '" + LogonUser + "') AND  USMA_REPORT_ID = (Select POS_REPORT_HEAD_ID from POS_REPORT_HEAD where POS_REPORT_HEAD_TITLE = 'Transaction'))"
                    ...
                    If posReportHeadTitles.Contains("Settings") = True Then
                        ...
                        stQuery = "Select distinct PSD_REPORT_TITLE from pos_report_detail where PSD_REPORT_ID in (select USMA_SUB_REPORT_ID from USER_SUBMENU_ACCESS where USMA_PRIV_YN='Y' AND USMA_UG_ID=(SELECT USER_GROUP_ID FROM MENU_USER WHERE USER_ID = '" + LogonUser + "') AND  USMA_REPORT_ID = (Select POS_REPORT_HEAD_ID from POS_REPORT_HEAD where POS_REPORT_HEAD_TITLE = 'Settings'))"
                    ...
                    If posReportHeadTitles.Contains("Masters") = True Then
                        ...
                        stQuery = "Select distinct PSD_REPORT_TITLE from pos_report_detail where PSD_REPORT_ID in (select USMA_SUB_REPORT_ID from USER_SUBMENU_ACCESS where USMA_PRIV_YN='Y' AND USMA_UG_ID=(SELECT USER_GROUP_ID FROM MENU_USER WHERE USER_ID = '" + LogonUser + "') AND  USMA_REPORT_ID = (Select POS_REPORT_HEAD_ID from POS_REPORT_HEAD where POS_REPORT_HEAD_TITLE = 'Masters'))"
                        'errLog.WriteToErrorLog("Setting", stQuery, "")
                        ds = db.SelectFromTableODBC(stQuery)
                        count = ds.Tables("Table").Rows.Count
                        i = 0
                    ...
                    If posReportHeadTitles.Contains("Reports") = True Then
                        ...
                        stQuery = "Select distinct PSD_REPORT_TITLE from pos_report_detail where PSD_REPORT_ID in (select USMA_SUB_REPORT_ID from USER_SUBMENU_ACCESS where USMA_PRIV_YN='Y' AND USMA_UG_ID=(SELECT USER_GROUP_ID FROM MENU_USER WHERE USER_ID = '" + LogonUser + "') AND  USMA_REPORT_ID = (Select POS_REPORT_HEAD_ID from POS_REPORT_HEAD where POS_REPORT_HEAD_TITLE = 'Reports'))"
                        'errLog.WriteToErrorLog("Setting", stQuery, "")
                        ds = db.SelectFromTableODBC(stQuery)
                        count = ds.Tables("Table").Rows.Count
                        i = 0
                        'Dim reportsarray As New ArrayList
                        While count > 0
                            reportsarray.Add(ds.Tables("Table").Rows.Item(i).Item(0).ToString)
                            count = count - 1
                            i = i + 1
                        End While
                        If reportsarray.Contains("End Of the Day Report") Then
                            Home.EndOfTheDayReportToolStripMenuItem.Enabled = True
                        Else
                            Home.EndOfTheDayReportToolStripMenuItem.Enabled = False
                        End If
                        If reportsarray.Contains("Daily Transaction Report") Then
