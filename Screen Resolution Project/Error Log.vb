...
Public Class Error_Log
    Public Sub WriteToErrorLog(ByVal msg As String, ByVal stkTrace As String, ByVal title As String)
        Try
            ...
            'check the file
            Dim fs As FileStream = New FileStream(Application.StartupPath & "\Errors\" & Location_Code & "_" & POSCounterNumber & "_errlog_" & (Date.Now.ToShortDateString).Replace("/", "_") & ".txt", FileMode.OpenOrCreate, FileAccess.ReadWrite)
            Dim s As StreamWriter = New StreamWriter(fs)
            s.Close()
            fs.Close()

            'log it
            Dim fs1 As FileStream = New FileStream(Application.StartupPath & "\Errors\" & Location_Code & "_" & POSCounterNumber & "_errlog_" & (Date.Now.ToShortDateString).Replace("/", "_") & ".txt", FileMode.Append, FileAccess.Write)
