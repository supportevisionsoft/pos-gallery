...
Public Class frmGVRep
    ...
    Private Sub getdatafunction()
        Try
            ...
            If cmbSm.Text = "All" Then
                ...
                stQuery = "SELECT  PINVGV_GVNO,PINVGV_GVCODE,PINVGV_GV_VALUE FROM OT_POS_INVOICE_GV,OT_INVOICE_HEAD WHERE PINVGV_INVH_SYS_ID=INVH_SYS_ID AND  INVH_LOCN_CODE = '" & strArrLoc(0) & "'  AND TO_DATE(PINVGV_CR_DT,'dd-MON-YY') >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') AND TO_DATE(PINVGV_CR_DT,'dd-MON-YY') <= TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') "
            Else
                stQuery = "SELECT  PINVGV_GVNO,PINVGV_GVCODE,PINVGV_GV_VALUE FROM OT_POS_INVOICE_GV,OT_INVOICE_HEAD WHERE PINVGV_INVH_SYS_ID=INVH_SYS_ID AND  INVH_LOCN_CODE = '" & strArrLoc(0) & "'  AND TO_DATE(PINVGV_CR_DT,'dd-MON-YY') >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy')  AND TO_DATE(PINVGV_CR_DT,'dd-MON-YY') <= TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') AND INVH_SM_CODE = '" & strArrSM(0) & "'"
            End If
    ...
    Private Sub btView_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btView.Click
        Try
            ...
            lblNoList.Text = vbCrLf & vbCrLf & vbCrLf & "Loading..."
            stDate = dtstDate.Value.ToString("dd-MMM-yyyy")
            edDate = dtendDate.Value.ToString("dd-MMM-yyyy")
            strArrLoc = cmbLocation.Text.Split("-")
            Dim strArr As Array
