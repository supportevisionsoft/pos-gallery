    ...
    Private Sub Call_salesreturn()
        Try
            If cmbSm.Text = "All" Then
                ...
                Query = "SELECT SUM(CSRI_QTY) AS QTY,SUM(CSRI_FC_VAL) AS VALUE FROM OT_CUST_SALE_RET_HEAD CSRH, OT_CUST_SALE_RET_ITEM CSRI WHERE CSRH.CSRH_SYS_ID=CSRI.CSRI_CSRH_SYS_ID AND CSRH_COMP_CODE = '001' AND CSRH_LOCN_CODE = '" & strArrLoc(0) & "' AND CSRH_DT >= '" & stDateval & "' AND CSRH_DT <= '" & endDateval & "' "
            Else
                Query = "SELECT SUM(CSRI_QTY) AS QTY,SUM(CSRI_FC_VAL) AS VALUE FROM OT_CUST_SALE_RET_HEAD CSRH, OT_CUST_SALE_RET_ITEM CSRI WHERE CSRH.CSRH_SYS_ID=CSRI.CSRI_CSRH_SYS_ID AND CSRH_COMP_CODE = '001' AND CSRH_LOCN_CODE = '" & strArrLoc(0) & "' AND CSRH_DT >= '" & stDateval & "' AND CSRH_DT <= '" & endDateval & "'   AND CSRH_SM_CODE = '" & cmbSm.Text & "'"
            End If
